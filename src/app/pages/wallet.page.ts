    ...
    <div class="min-h-screen bg-background text-foreground">
      ...
      <main class="mx-auto max-w-7xl space-y-8 px-4 py-8 md:px-6">
        <header class="flex items-center justify-between gap-4">
          <div>
            <div class="text-xs font-mono uppercase tracking-[0.3em] text-primary">{{ 'wallet.kicker' | transloco }}</div>
            <h1 class="mt-2 text-3xl font-black tracking-tight md:text-4xl">{{ 'wallet.title' | transloco }}</h1>
          </div>
          <button
        ...
        @if (wallet()) {
          <section class="grid grid-cols-2 gap-3 md:grid-cols-4">
            <div class="surface-panel rounded-xl p-4">
              <div class="text-xs font-mono uppercase text-muted-foreground">{{ 'wallet.balance' | transloco }}</div>
              <div class="mt-1 text-2xl font-black text-primary">{{ wallet()!.balance }}</div>
            </div>
            <div class="surface-panel rounded-xl p-4">
              <div class="text-xs font-mono uppercase text-muted-foreground">{{ 'wallet.coinsWon' | transloco }}</div>
              <div class="mt-1 text-2xl font-black">{{ wallet()!.totalCoinsWon }}</div>
            </div>
            <div class="surface-panel rounded-xl p-4">
              <div class="text-xs font-mono uppercase text-muted-foreground">{{ 'wallet.coinsSpent' | transloco }}</div>
              <div class="mt-1 text-2xl font-black">{{ wallet()!.totalCoinsSpent }}</div>
            </div>
            <div class="surface-panel rounded-xl p-4">
              <div class="text-xs font-mono uppercase text-muted-foreground">{{ 'wallet.coinsPurchased' | transloco }}</div>
        ...
        <section class="space-y-3">
          <h2 class="text-xs font-mono uppercase tracking-[0.3em] text-muted-foreground">{{ 'wallet.transactionHistory' | transloco }}</h2>
          <div class="surface-panel overflow-hidden rounded-xl">
            @if (transactions().length === 0) {
              <div class="p-4 text-sm font-mono text-muted-foreground">{{ 'wallet.noTransactions' | transloco }}</div>
            } @else {
              <div class="divide-y divide-border/60">
                @for (tx of transactions(); track tx.id) {
                  <div class="grid gap-2 px-4 py-3 md:grid-cols-[160px_1fr_auto_auto] md:items-center">
                    ...
                    <span class="truncate text-sm">{{ tx.reason || ('wallet.noReason' | transloco) }}</span>
