  ...
  template: `
    <div class="h-full border-t border-border/60 bg-background/55">
      <header class="flex items-center justify-between gap-3 border-b border-border/60 px-4 py-4 md:px-6">
        <div>
          <div class="text-ui-kicker text-muted-foreground">{{ 'admin.purchases.kicker' | transloco }}</div>
          <h1 class="mt-1 text-2xl font-black tracking-tight">{{ 'admin.purchases.title' | transloco }}</h1>
        </div>
      ...
      <section class="px-4 py-4 md:px-6">
        <div class="surface-panel overflow-hidden rounded-xl">
          @if (purchases().length === 0) {
            <div class="p-4 text-sm font-mono text-muted-foreground">{{ 'admin.purchases.empty' | transloco }}</div>
          } @else {
            <div class="divide-y divide-border/60">
              @for (purchase of purchases(); track purchase.id) {
                <article class="grid gap-3 px-4 py-4 lg:grid-cols-[1fr_auto]">
                  <div class="space-y-1">
                    <div class="font-semibold">{{ purchase.coinsReceived }} {{ 'common.coins' | transloco }} / {{ purchase.amountPaid }} {{ purchase.currency }}</div>
                    <div class="text-xs font-mono text-muted-foreground">
                      {{ purchase.paymentMethod }} / {{ purchase.paymentDestination }}
                    </div>
                    <div class="text-xs font-mono text-muted-foreground">
                      {{ 'admin.purchases.sender' | transloco }}: {{ purchase.senderPhone || '-' }} / {{ 'admin.purchases.reference' | transloco }}: {{ purchase.paymentReference || '-' }}
                    </div>
                    @if (purchase.voucherCode) {
                      <div class="text-xs font-mono text-primary">
                        {{ 'admin.purchases.voucher' | transloco }}: {{ purchase.voucherCode }} ({{ 'admin.purchases.discount' | transloco }} {{ purchase.voucherDiscountAmount }} {{ purchase.currency }})
                      </div>
                    }
                  ...
                  <div class="flex flex-wrap items-center gap-2">
                    <button class="btn-game rounded-md border border-primary/40 px-3 py-2 text-xs font-bold uppercase tracking-wider text-primary hover:bg-primary/10 disabled:opacity-40" type="button" [disabled]="busy() || purchase.paymentStatus !== 'pending_admin_review'" (click)="approve(purchase)">
                      {{ 'admin.purchases.approve' | transloco }}
                    </button>
                    <button class="btn-game rounded-md border border-destructive/40 px-3 py-2 text-xs font-bold uppercase tracking-wider text-destructive hover:bg-destructive/10 disabled:opacity-40" type="button" [disabled]="busy() || purchase.paymentStatus !== 'pending_admin_review'" (click)="reject(purchase)">
                      {{ 'admin.purchases.reject' | transloco }}
                    </button>
                  </div>
                </article>
              }
...
export class AdminPurchasesPage {
  ...
  async reload(): Promise<void> {
    try {
      ...
      this.statusMessage.set(error instanceof Error ? error.message : this.i18n.t('admin.purchases.errors.load'));
    }
  }
  ...
  async approve(purchase: PurchaseSnapshot): Promise<void> {
    ...
    this.statusMessage.set(null);
    try {
      await this.supabase.adminApprovePurchase(purchase.id, this.i18n.t('admin.purchases.approvedNote'));
      await this.reload();
      this.statusMessage.set(this.i18n.t('admin.purchases.approved', { id: purchase.id }));
    } catch (error) {
      this.statusMessage.set(error instanceof Error ? error.message : this.i18n.t('admin.purchases.errors.approve'));
    } finally {
      this.busy.set(false);
    }
  }
  ...
  async reject(purchase: PurchaseSnapshot): Promise<void> {
    const reason = globalThis.prompt(this.i18n.t('admin.purchases.rejectionReason'));
    ...
    try {
      ...
      await this.reload();
      this.statusMessage.set(this.i18n.t('admin.purchases.rejected', { id: purchase.id }));
    } catch (error) {
      this.statusMessage.set(error instanceof Error ? error.message : this.i18n.t('admin.purchases.errors.reject'));
    } finally {
      this.busy.set(false);
    }
  }
